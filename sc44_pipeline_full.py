...
def build_row(rel_name, stage, orig44k, attacked_audio, model, sr, desync,
    ...
    if not desync:
        min_len = min(len(orig44k), len(attacked_audio))
        sisnr = si_snr(orig44k[:min_len], attacked_audio[:min_len])
        visqol_score = compute_visqol(orig44k[:min_len], attacked_audio[:min_len], sr)
        odg_score = compute_odg(orig44k[:min_len], attacked_audio[:min_len], sr)
        ph = phase_consistency_metrics(orig44k, attacked_audio, sr)
        plv, phase_diff, gd_mae = ph["PLV"], ph["PhaseDiff_rad"], ph["GroupDelayMAE"]

    if spec_out is not None:
        min_len = min(len(orig44k), len(attacked_audio))
        save_diff_spectrogram(orig44k[:min_len], attacked_audio[:min_len], sr, spec_out)
